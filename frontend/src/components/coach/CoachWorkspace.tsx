...
import { BarChart3, Bot, ChevronDown, ChevronRight, HelpCircle, Loader2, Menu, MessageSquarePlus, Paperclip, Send, Sparkles, X } from 'lucide-react';
...
export function CoachWorkspace() {
  ...
  const { watchJob, status } = useAnalysisStatus(user?.id);
  ...
  const profileData = useMemo(() => ({
    ...
    bottleneck: firstProfileItem(profile?.primary_weaknesses, 'Analyze games to identify'),
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      ...
      watchJob(result.job_id ?? undefined, {
        ...
        onError: () => toast.error('Analysis stopped before it completed. Please try again.'),
      });
  ...
  return <div className="min-h-screen bg-[#0a0a0a] font-sans text-[#e5e2e1]">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-80 flex-col bg-[#201f1f] p-7 lg:flex">
      ...
      <div className="mt-auto border-t border-[#3c4a42] pt-5"><button type="button" onClick={() => setAnalysisOpen(true)} className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#10b981] px-4 py-3 font-mono text-sm font-semibold text-[#00422b] transition-opacity hover:opacity-90"><BarChart3 className="h-4 w-4" /> Analyze Games</button><div className="mt-7 space-y-4 px-2 text-sm text-[#bbcabf]"><span className="flex items-center gap-3"><HelpCircle className="h-4 w-4" /> Help</span><span className="flex items-center gap-3"><Sparkles className="h-4 w-4" /> ChessRun Coach</span></div></div>
    </aside>
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-[#3c4a42] bg-[#131313]/95 px-5 backdrop-blur lg:hidden"><Brand compact /><div className="flex items-center gap-2"><button type="button" aria-label="Analyze games" onClick={() => setAnalysisOpen(true)} className="rounded-md p-2 text-brand-primary hover:bg-[#201f1f]"><BarChart3 className="h-5 w-5" /></button><button type="button" aria-label="Open menu" onClick={() => setMobileMenuOpen((open) => !open)} className="rounded-md p-2 text-[#bbcabf] hover:bg-[#201f1f]"><Menu className="h-5 w-5" /></button></div></header>
    {mobileMenuOpen && <div className="fixed inset-x-0 top-16 z-40 border-b border-[#3c4a42] bg-[#201f1f] p-4 lg:hidden"><button type="button" onClick={() => void handleNewChat()} className="flex w-full items-center gap-3 rounded-lg bg-[#2a2a2a] p-3 text-left text-sm"><MessageSquarePlus className="h-4 w-4 text-brand-primary" /> New Chat</button></div>}
    <main className="mx-auto flex min-h-screen max-w-[1280px] flex-col px-4 pb-32 pt-5 sm:px-6 lg:ml-80 lg:px-12 lg:pt-12"><section className="mx-auto w-full max-w-[940px]"><button type="button" onClick={() => setProfileOpen((open) => !open)} className="flex w-full items-center justify-between rounded-xl border border-t-2 border-[#3c4a42] border-t-brand-primary bg-[#171717] px-5 py-4 text-left"><span className="flex min-w-0 items-center gap-3"><ChevronDown className={`h-4 w-4 shrink-0 text-brand-primary transition-transform ${profileOpen ? 'rotate-180' : ''}`} /><span className="font-mono text-sm font-semibold">Your Playing Profile</span><span className="hidden font-mono text-xs text-[#bbcabf] sm:inline">{profileData.games} Games</span><span className="hidden font-mono text-xs text-[#bbcabf] sm:inline">{profileData.patterns} Patterns</span></span><span className="hidden text-xs text-[#bbcabf] md:block">Updated {profileData.updated}</span></button>{profileOpen && <div className="grid grid-cols-2 gap-5 border-x border-b border-[#3c4a42] bg-[#171717] p-5 text-sm md:grid-cols-4"><ProfileMetric label="Games Analyzed" value={String(profileData.games)} /><ProfileMetric label="Patterns Identified" value={String(profileData.patterns)} /><ProfileMetric label="Strongest Area" value={profileData.strength} positive /><ProfileMetric label="Biggest Bottleneck" value={profileData.bottleneck} warning /></div>}</section>
      <section className="mx-auto flex w-full max-w-[940px] flex-1 flex-col pt-10">{messages.length > 0 && <div className="space-y-6">{messages.map((message) => <ChatMessage key={message.id} role={message.role} content={message.content} />)}</div>}{isTyping && <div className="mt-5 flex items-center gap-2 text-sm text-[#bbcabf]"><Bot className="h-4 w-4 text-brand-primary" /><span>Coach is thinking...</span></div>}{error && <p className="mt-4 text-sm text-brand-error">{error}</p>}<div className={`${messages.length ? 'mt-12' : 'my-auto pb-14'} text-center`}>{messages.length === 0 && <><Bot className="mx-auto mb-5 h-9 w-9 text-brand-primary" /><h1 className="text-2xl font-semibold sm:text-[32px]">What should we focus on next?</h1><p className="mx-auto mt-4 max-w-xl text-base leading-7 text-[#bbcabf]">{profile?.profile_summary || 'Analyze your games and I will build a focused coaching profile around the improvement that matters most.'}</p></>}<div className="mx-auto mt-8 grid max-w-[940px] grid-cols-1 gap-4 md:grid-cols-2">{STARTERS.map(({ title, prompt }) => <button key={title} type="button" onClick={() => setInput(prompt)} className="rounded-lg border border-[#262626] bg-[#171717] p-5 text-left transition-colors hover:border-[#3f3f46] hover:bg-[#201f1f]"><span className="font-mono text-sm font-semibold text-[#e5e2e1]">{title}</span><p className="mt-2 text-sm text-[#bbcabf]">&quot;{prompt}&quot;</p></button>)}</div></div></section>
    </main>
