...
function friendlyBottleneck(items: unknown[] | null | undefined): string {
  ...
  if (/opening-phase acpl|high opening acpl/i.test(value))
    return "Your openings are where we can make the quickest gains. Let's work on reaching the middlegame with a clearer plan.";
  if (/middlegame acpl|high middlegame acpl/i.test(value))
    return "The middlegame is our best opportunity right now. Let's make your plans and calculations more consistent.";
  if (/endgame acpl|high endgame acpl/i.test(value))
    return "Your endgames offer the clearest path forward. Let's sharpen the technique that turns close games into points.";
  return value;
}
...
export function CoachWorkspace() {
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      ...
      watchJob(result.job_id ?? undefined, {
        ...
        onError: (analysisFailure) =>
          toast.error(`Analysis needs attention: ${analysisFailure.message}`),
      });
    } catch (requestError: unknown) {
      const detail = (requestError as { response?: { data?: { detail?: string } } })?.response
        ?.data?.detail;
  ...
  if (!user)
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#0a0a0a] px-6 text-center text-content-muted">
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] font-sans text-[#e5e2e1]">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-80 flex-col bg-[#201f1f] p-7 lg:flex">
        ...
        <div className="mt-7 min-h-0 flex-1 overflow-y-auto">
          ...
          {recentSessions.length ? (
            <div className="space-y-1">
              {recentSessions.map((session) => (
                <button
                  ...
                  className={`w-full rounded-md px-2 py-2 text-left text-sm transition-colors hover:bg-[#2a2a2a] hover:text-[#e5e2e1] ${
            ...
            <p className="px-2 text-sm leading-6 text-[#bbcabf]">
              Your coaching conversations will appear here as you continue working with your coach.
            </p>
          )}
        </div>
        <div className="mt-auto border-t border-[#3c4a42] pt-5">
          <button
            type="button"
            onClick={() => setAnalysisOpen(true)}
            disabled={isTracking}
            className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#10b981] px-4 py-3 font-mono text-sm font-semibold text-[#00422b] transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
          >
          ...
          <div className="mt-7 space-y-4 px-2 text-sm text-[#bbcabf]">
            ...
            <span className="flex items-center gap-3">
              <Sparkles className="h-4 w-4" /> ChessRun Coach
            </span>
          </div>
        </div>
      </aside>
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-[#3c4a42] bg-[#131313]/95 px-5 backdrop-blur lg:hidden">
        <Brand compact />
        <div className="flex items-center gap-2">
          <button
            type="button"
            aria-label="Analyze games"
            onClick={() => setAnalysisOpen(true)}
            disabled={isTracking}
            className="rounded-md p-2 text-brand-primary hover:bg-[#201f1f] disabled:opacity-60"
          >
          ...
          <button
            ...
            className="rounded-md p-2 text-[#bbcabf] hover:bg-[#201f1f]"
          >
      ...
      <main className="mx-auto flex min-h-screen max-w-[1280px] flex-col px-4 pb-32 pt-5 sm:px-6 lg:ml-80 lg:px-12 lg:pt-12">
        <section className="mx-auto w-full max-w-[940px]">
          <button
            type="button"
            onClick={() => setProfileOpen((open) => !open)}
            className="flex w-full items-center justify-between rounded-xl border border-t-2 border-[#3c4a42] border-t-brand-primary bg-[#171717] px-5 py-4 text-left"
          >
            <span className="flex min-w-0 items-center gap-3">
              <ChevronDown
                className={`h-4 w-4 shrink-0 text-brand-primary transition-transform ${
                  profileOpen ? 'rotate-180' : ''
                }`}
              />
              <span className="font-mono text-sm font-semibold">Your Playing Profile</span>
              <span className="hidden font-mono text-xs text-[#bbcabf] sm:inline">
            ...
            <span className="hidden text-xs text-[#bbcabf] md:block">
              Updated {profileData.updated}
            </span>
          </button>
          {profileOpen && (
            <div className="grid grid-cols-2 gap-5 border-x border-b border-[#3c4a42] bg-[#171717] p-5 text-sm md:grid-cols-4">
              <ProfileMetric label="Games Analyzed" value={String(profileData.games)} />
              <ProfileMetric label="Patterns Identified" value={String(profileData.patterns)} />
              <ProfileMetric label="Strongest Area" value={profileData.strength} positive />
              <ProfileMetric label="Biggest Bottleneck" value={profileData.bottleneck} warning />
            </div>
          )}
          {(status || analysisError) && (
            <AnalysisStatusCard
              ...
              onAnalyzeAgain={() => setAnalysisOpen(true)}
            />
          )}
        </section>
        <section className="mx-auto flex w-full max-w-[940px] flex-1 flex-col pt-10">
          {isRestoringSession && messages.length === 0 ? (
            <div className="flex flex-1 items-center justify-center text-[#bbcabf]">
            ...
            <>
              ...
              {isTyping && (
                <div className="mt-5 flex items-center gap-2 text-sm text-[#bbcabf]">
                  ...
                  <div className="mx-auto mt-8 grid max-w-[940px] grid-cols-1 gap-4 md:grid-cols-2">
                    {STARTERS.map(({ title, prompt }) => (
                      <button
                        ...
                        className="rounded-lg border border-[#262626] bg-[#171717] p-5 text-left transition-colors hover:border-[#3f3f46] hover:bg-[#201f1f]"
                      >
                        <span className="font-mono text-sm font-semibold text-[#e5e2e1]">{title}</span>
                        <p className="mt-2 text-sm text-[#bbcabf]">&quot;{prompt}&quot;</p>
                      </button>
                    ))}
                  </div>
                )}
      ...
      <form
        onSubmit={handleSend}
        className="fixed bottom-0 left-0 right-0 z-30 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a] to-transparent px-4 pb-5 pt-10 lg:pl-80"
      >
        <div className="mx-auto max-w-[1000px]">
          <div className="flex items-center gap-2 rounded-xl border border-[#262626] bg-[#171717] px-3 py-2 focus-within:border-brand-primary">
            ...
            <button
              ...
              className="p-2 text-[#10b981] disabled:opacity-40"
            >
              <Send className="h-5 w-5" />
            </button>
          </div>
          <p className="mt-3 text-center text-xs text-[#bbcabf]">
            ChessRun AI can make mistakes. Always review critical analysis.
          </p>
        </div>
      </form>
      {analysisOpen && (
...
function Brand({ compact = false }: { compact?: boolean }) {
  return (
    <div className="flex items-center gap-3">
      <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-brand-primary/15 text-brand-primary">
        <Bot className="h-5 w-5" />
      </div>
      <div>
        <p className="text-xl font-semibold text-brand-primary">ChessRun</p>
        {!compact && <p className="text-xs text-[#bbcabf]">AI Grandmaster Coach</p>}
      </div>
    </div>
  );
}
...
function ProfileMetric({
  ...
  return (
    <div>
      <p className="font-mono text-[11px] uppercase text-[#bbcabf]">{label}</p>
      <p className={`mt-2 text-sm ${positive ? 'text-brand-primary' : warning ? 'text-[#ffb3af]' : 'text-[#e5e2e1]'}`}>
        {value}
      </p>
    </div>
  );
...
function AnalysisStatusCard({
  ...
  const title = isRunning
    ? `Analyzing game ${activeNumber} of ${status?.total_games ?? 0}`
    : isCancelled
      ? 'Analysis cancelled'
      : isFailure
  ...
  const detail = isRunning
    ? `${processed} completed. Your coach will update your Playing Profile when the job finishes.`
    : isCancelled
      ? `${status?.completed_games ?? 0} completed games were kept. You can start another analysis whenever you are ready.`
      : isFailure
        ? status?.last_error || error?.message || 'The analysis worker could not process these games.'
        : isPartial
          ? `${status?.completed_games ?? 0} games completed; ${status?.failed_games ?? 0} need attention.`
          : `${status?.completed_games ?? 0} games are now available for coaching.`;
  const Icon = isRunning ? Clock3 : isFailure ? AlertTriangle : CheckCircle2;
  const tone = isFailure
    ? 'border-[#fc7c78]/50 bg-[#93000a]/20 text-[#ffb4ab]'
    : isRunning
      ? 'border-brand-primary/50 bg-brand-primary/10 text-brand-primary'
      : 'border-brand-primary/50 bg-[#10b981]/10 text-[#6ffbbe]';

  return (
    <div className={`mt-4 rounded-lg border p-4 ${tone}`} role="status">
      <div className="flex items-start gap-3">
        <Icon className={`mt-0.5 h-5 w-5 shrink-0 ${isRunning ? 'animate-pulse' : ''}`} />
        <div className="min-w-0 flex-1">
          <p className="font-mono text-sm font-semibold">{title}</p>
          <p className="mt-1 text-sm leading-6 text-[#e5e2e1]">{detail}</p>
          {isRunning && (
            <>
              <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-black/25">
                <div
                  className="h-full rounded-full bg-brand-primary transition-all"
                  style={{ width: `${Math.max(4, status?.progress_percent ?? 0)}%` }}
                />
              </div>
              <button
                type="button"
                onClick={onCancel}
                disabled={cancelling}
                className="mt-3 font-mono text-xs font-semibold text-[#ffb4ab] underline underline-offset-4 disabled:opacity-50"
              >
          ...
          {(isFailure || isCancelled) && (
            <button
              ...
              className="mt-3 font-mono text-xs font-semibold underline underline-offset-4"
            >
              Analyze again
            </button>
          )}
...
function ChatMessage({ role, content }: { role: string; content: string }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        className={`max-w-[85%] ${
          isUser ? 'rounded-xl border border-[#262626] bg-[#171717]' : 'border-t-2 border-brand-primary'
...
function AnalysisModal({
  ...
  customDays: number;
  starting: boolean;
  onSelect: (range: AnalysisRange) => void;
  onCustomDays: (days: number) => void;
  onClose: () => void;
  onStart: () => void;
}) {
  const choices: Array<{ label: string; value: AnalysisRange; description: string }> = [
    { label: 'Analyze All Games', value: 'all', description: 'Every game from your linked account' },
    { label: 'Last 7 Days', value: 7, description: 'Recent games' },
    { label: 'Last 30 Days', value: 30, description: 'A broader recent sample' },
    { label: 'This Month', value: 'month', description: 'Games played this calendar month' },
  ];
  return (
    <div
      ...
      aria-label="Analyze games"
    >
      <div className="w-full max-w-lg rounded-t-2xl border border-[#3c4a42] bg-[#201f1f] p-6 shadow-2xl sm:rounded-xl">
        <div className="mb-6 flex items-start justify-between">
          <div>
            <p className="font-mono text-xs uppercase tracking-wider text-brand-primary">
              Select timeframe
            </p>
            <h2 className="mt-2 text-2xl font-semibold">Analyze Games</h2>
            <p className="mt-2 text-sm leading-6 text-[#bbcabf]">
              Choose the games that should inform your coaching profile.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-2">
            ...
            <button
              ...
              className={`flex w-full items-center justify-between rounded-lg border p-4 text-left ${
                range === choice.value
                  ...
                  : 'border-[#262626] bg-[#171717] hover:border-[#3f3f46]'
              }`}
            >
              <span>
                <span className="block font-medium">{choice.label}</span>
                <span className="mt-1 block text-xs text-[#bbcabf]">{choice.description}</span>
              </span>
          ...
          >
            ...
            {range === 'custom' && (
              <label className="mt-3 block text-sm text-[#bbcabf]">
                Days to analyze
                <input
                  type="number"
                  min="1"
                  max="365"
                  value={customDays}
                  onChange={(event) => onCustomDays(Math.max(1, Number(event.target.value)))}
                  className="mt-2 w-full rounded-md border border-[#3c4a42] bg-[#131313] px-3 py-2 text-[#e5e2e1] outline-none focus:border-brand-primary"
                />
              </label>
            )}
          </div>
        ...
        <div className="mt-6 flex justify-end gap-3">
          <button
            ...
            onClick={onClose}
            disabled={starting}
            className="px-4 py-2 text-sm text-[#bbcabf] hover:text-[#e5e2e1]"
          >
            Cancel
          </button>
          <button
            ...
            onClick={onStart}
            disabled={starting}
            className="flex items-center gap-2 rounded-lg bg-[#10b981] px-4 py-2 text-sm font-semibold text-[#00422b] disabled:opacity-60"
          >
