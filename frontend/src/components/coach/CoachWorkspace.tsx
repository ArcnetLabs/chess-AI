...
function AnalysisStatusCard({ status, error, onAnalyzeAgain }: { status: AnalysisJobStatus | null; error: Error | null; onAnalyzeAgain: () => void }) { const state = status?.status; const isRunning = state === 'pending' || state === 'running'; const isFailure = state === 'failed' || !!error; const isPartial = state === 'partial'; const processed = status ? status.completed_games + status.failed_games : 0; const activeNumber = status?.total_games ? Math.min(processed + 1, status.total_games) : 0; const title = isRunning ? `Analyzing game ${activeNumber} of ${status?.total_games ?? 0}` : isFailure ? 'Analysis could not complete' : isPartial ? 'Analysis completed with issues' : 'Analysis complete'; const detail = isRunning ? `${processed} completed. Your coach will update your Playing Profile when the job finishes.` : isFailure ? (status?.last_error || error?.message || 'The analysis worker could not process these games.') : isPartial ? `${status?.completed_games ?? 0} games completed; ${status?.failed_games ?? 0} need attention.` : `${status?.completed_games ?? 0} games are now available for coaching.`; const Icon = isRunning ? Clock3 : isFailure ? AlertTriangle : CheckCircle2; const tone = isFailure ? 'border-[#fc7c78]/50 bg-[#93000a]/20 text-[#ffb4ab]' : isRunning ? 'border-brand-primary/50 bg-brand-primary/10 text-brand-primary' : 'border-brand-primary/50 bg-[#10b981]/10 text-[#6ffbbe]'; return <div className={`mt-4 rounded-lg border p-4 ${tone}`} role="status"><div className="flex items-start gap-3"><Icon className={`mt-0.5 h-5 w-5 shrink-0 ${isRunning ? 'animate-pulse' : ''}`} /><div className="min-w-0 flex-1"><p className="font-mono text-sm font-semibold">{title}</p><p className="mt-1 text-sm leading-6 text-[#e5e2e1]">{detail}</p>{isRunning && <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-black/25"><div className="h-full rounded-full bg-brand-primary transition-all" style={{ width: `${Math.max(4, status?.progress_percent ?? 0)}%` }} /></div>}{isFailure && <button type="button" onClick={onAnalyzeAgain} className="mt-3 font-mono text-xs font-semibold underline underline-offset-4">Analyze again</button>}</div></div></div>; }
