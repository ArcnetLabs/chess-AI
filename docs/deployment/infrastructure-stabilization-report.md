...
| 7 | Verify Celery worker startup | **Done** | Render `worker` service + `scripts/verify_infrastructure.py` import check |
...
4. **pytest:** Still uses in-memory SQLite via `TESTING=1` in `conftest.py` — isolated from production paths.
